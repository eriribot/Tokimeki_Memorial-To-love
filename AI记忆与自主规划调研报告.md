# AI 记忆、变量解析与自主规划调研报告

> 开工前的模块、协议与接口规格见 `本地记忆与上下文系统实施规格.md`；当前运行真相以 `MODULES.md` 为准。浏览器本地纯文本摘要与候选审查已经接通，候选使用 `memory-summary-archive:v3`，新候选的 `facts` 固定为空；加载器不读取或迁移遗留 v1/v2。Tavern 记忆侧档、剧情上下文注入、变量解析与 shujuku 仍未接通。

## 结论摘要

本项目不应直接复制 Generative
Agents 一类系统的“自主规划”。那类系统的规划器可以决定下一步观察、行动和反思；本项目的日期、AP、主线触发、关系结算和存档恢复必须由本地确定性状态决定。模型可以提供叙事候选、语义信号和回忆候选，但不能获得推进游戏世界的权限。

本轮采用的最小架构是：

```text
Zustand + GameSnapshot v2
  日期 / AP / 玩家属性 / 角色关系 / 主线游标 / 技能

MessageArchive v2
  已接受和解析失败的本地原文楼层

Context Projection
  当前快照投影 + 当前主线历史窗口 + 生成提示 + 世界书引用

AI / 副 API
  仅输出正文候选、语义信号或摘要候选
```

本地快照、原文归档和上下文预览先形成可检查闭环；真实 shujuku、ACU、hidden host
floors 和数据库写入留到本地合同通过后再接。

## 为什么自主规划不能直接照搬

### 1. 权威状态不同

Generative
Agents 的规划是为了让代理在环境中持续行动，计划本身可以改变环境。本项目的日期、AP、事件完成、当前幕和关系变量是游戏规则，不是模型意见。让规划器决定“今天去哪里”“是否推进一天”会绕过
`gameStore` 和严格快照恢复，导致同一输入无法重放。

### 2. 玩家意图不可被代理替代

这是玩家做选择的约会游戏，而不是代理自己生活的模拟。NPC 可以有目标和情绪，但玩家行动必须先经过选项选择和本地结算；AI 只能把已经结算的结果演成 GAL 正文。否则模型会替玩家邀约、失约、消费 AP，玩家无法解释关系变化。

### 3. 记忆里的“观察”不等于游戏事实

自主代理通常把观察流作为世界事实，再由反思生成高层记忆。本项目的候选正文可能解析失败、被玩家拒绝、被重新生成或属于未采用楼层。只有本地接受并绑定到
`floorId` 的内容才可进入连续性上下文；摘要绝不能反向写日期、AP、好感或事件状态。

### 4. 计划与异步存档存在竞态

模型请求返回前，玩家可能换档、重抽当前楼层或删除候选。自主规划没有本项目的 `saveUuid + revision + source fingerprint`
约束时，迟到结果会污染新存档。任何副 API 结果都必须绑定基准版本，过期结果直接丢弃。

### 5. 规划目标与主线合同冲突

本项目主线按照 `eventId + actId`
和日期/AP 触发，世界书条目、场景序列、立绘规则和正文协议都有本地校验。自主规划器可能为了“合理”而跳过必经场景、虚构人物或提前完成事件，这与
`storyPresentation` 和 `storyPersistence` 的确定性合同冲突。

### 6. 测试与成本不允许黑箱决策

主线和约会需要能够重放、回归、保存、读取和比较 fallback。若规划、变量写回和摘要都由模型决定，就无法区分剧情质量问题、规则问题和异步问题，也无法在没有副 API 的玩家环境中提供稳定体验。

## 成熟方案的可复用部分

| 方案              | 可以借鉴                                                              | 不直接复制                                          |
| ----------------- | --------------------------------------------------------------------- | --------------------------------------------------- |
| AI Dungeon Memory | 最近原文、自动摘要、可编辑记忆、Context Viewer                        | 不把 RPG 数值状态藏进摘要；不让摘要成为唯一上下文   |
| Generative Agents | 记忆流、近期性/重要性/相关性检索、带来源的反思                        | 不复制自主行动和自主日程；不让反思写游戏状态        |
| Inworld           | 情绪、目标、知识可见性、关系状态分层                                  | 不把所有角色状态合成一个自由文本 Memory             |
| Tokimeki 关系机制 | 友好度、心动度、伤心度和危机派生                                      | 不把单一 affection 当成所有关系；不让 AI 直接改数值 |
| islandmilfcode    | 当前轮变量解析与长期摘要分离、白名单、限幅、证据引用、取消和 fallback | 不复制多表 MemoryDB、并行权威和摘要回写变量         |

参考：

- [AI Dungeon Memory System](https://help.aidungeon.com/faq/the-memory-system)
- [AI Dungeon Context Management](https://help.aidungeon.com/how-do-i-manage-context)
- [Generative Agents](https://arxiv.org/abs/2304.03442)
- [Inworld Relation State](https://docs.inworld.ai/unreal-engine/runtime/character-reference/InworldGraphRuntimeData_RelationState/InworldGraphRuntimeData_RelationState)
- [Tokimeki relationship reverse engineering](https://gcgx.games/tokimeki/friendship.html)

## 对当前架构的映射

`save/snapshot.ts` 已是日期、AP、主线、玩家和角色状态的 schema-v2 权威；`message/protocol.ts` 已是与
`saveUuid + saveRevision`
配对的本地原文镜像。新增一套关系或记忆 store 会制造重复权威，因此上下文预览只读取现有 store 和消息镜像，不复制状态。

当前生成器原本在 `tavernStoryGeneration.ts`
内部截取最后 6 个 message 对象。此次将这个选择抽成共享投影，使 Tavern 调用和 UI 预览使用同一份
`chatHistory`。这里的 6 指 6 条消息，不是 6 个剧情版本；每个采用版本保存一条 User 提示和一条 Assistant 原文，所以窗口最多容纳 3 个完整版本。

现有实现还有一处跨集缺口：`getPreviousActiveStoryFloors()` 只返回同一集的前幕，`selectGenerationMessages()` 又按当前
`eventId` 过滤。生成第二集第三幕前，第一集两幕和第二集前两幕本应共有 4 个采用版本、8 条消息（数组编号 0～7），但旧投影只会看到第二集前两幕的 4 条消息。当前跨集投影固定保留最后 6 条；最早的 2 条成为一个可总结楼层，但不足固定的 2 楼层批次，不会提前请求副 API。完成第二集第三幕后总计 5 个楼层，第一集两幕才恰好形成首个小总结批次。

关系系统仍需后续单独实现 `friendship / romance / hurt` 和本地结算，但不在这轮的本地记忆闭环中偷偷加入。`affection`
暂时继续作为现有 UI 字段，不能宣称 Tokimeki 变量已经接通。

## 本轮交付与边界

已实现的目标：

- 复用现有 GameSnapshot v2 和 MessageArchive v2，不引入迁移分支。
- 生成器与预览共用精确的最近消息投影。
- “目录”菜单展示当前本地快照、原文消息、生成提示、历史窗口和世界书引用；原“数据”入口停用。
- 预览明确标出 fallback、Tavern generate、宿主消息、shujuku/database 的不同接通状态。
- 副 API 返回纯文本摘要；浏览器 `memory-summary-archive:v3` 负责本地候选封装与玩家接受、编辑、拒绝和重新生成。标题和来源元数据由本地生成，新候选 `facts: []`。

仍未实现的边界：

- 没有创建真实 Tavern hidden floors。
- 没有触发 `MESSAGE_SENT`、ACU、shujuku 或数据库写入。
- 没有让副 API 写入 friendship、romance、hurt、AP、日期或事件。
- 没有把已接受摘要注入剧情，也没有实现 Tavern 记忆侧档、变量解析或向量检索。

## 下一轮验收建议

在 4 月 8 日空挡期，从 4 月 7 日第一集结束状态打开“目录”，确认快照、原文和关系字段可见；有当前主线生成时，确认预览里的提示和 6 条历史消息与实际生成调用一致。随后关闭 Tavern
generate，使用 fallback 重复保存/读取，验证本地快照和原文归档仍可恢复。

人工验收通过前，预览只能标记为“本地状态演示”，不能升级为真实 shujuku/宿主集成。

## 2026-07-23 补充判断与待办

### 关系提示词：借结构，不照搬正文

`islandmilfcode/relationship.ts` 值得借的是分层：先把好感限制在 0～100，再选择阶段反应；身份锚点、称呼、关系反应和角色审计分别生成，原作关系也明确不自动转移给 User。这比把“她现在喜欢谁”塞回人物世界书稳得多。

不适合直接复制的部分也很明显：文件同时装了大量角色专属人格、绝对规则、旧情轴和长篇行为审计，提示词会越来越长，还会与本项目人物世界书重复。To LOVE-Ru 版本只应注入短叠层：当前角色、对 User 的已结算关系值、对应阶段的思考倾向、称呼规则和禁止越级。人物性格仍由人物条目负责，关系叠层不能复述整张角色卡。

### 认知：由代码按已采用剧情推导

人物条目里目前直接写了“第一集认知边界”“第一至第二集认知边界”。整份人物条目被当前幕选中时，这些未来说明也会一起进入扫描，容易让模型提前知道后幕事实。

后续应新增独立认知投影，按稳定的 `eventId + actId` 和当前采用楼层判断角色已经知道什么。生成一幕前只注入上一幕结束时已确认的事实；本幕新知识由剧情条目在正文中演出，采用后才成为下一幕可用认知。认知从现有剧情档案派生，不再另建一份可漂移的进度权威。

### 两步完成记忆闭环

1. Step 1：把全部已采用主线版本按全局剧情顺序排好，每个版本固定为 User/Assistant 一对。预览同时显示“总消息数、最近窗口、待总结数”，超过 6 条后只把完整旧对加入待总结队列，不能从中间切断一对。跨集选择、重新生成和 schema-v2 读取规则要一起修改，避免旧档被同版本代码误判。
2. Step 2：配置与浏览器本地摘要链已经完成。“系统设定”现有“AI 记忆设定”页，使用 `window_kani.png` 作为完整设置页外壳、原生 `midashi_op.png` 作为左上标题名牌，不再混用 `window_system.png`；地址沿用酒馆“自定义（兼容 OpenAI）”的含义，作为完整 API 基址直接追加 `/models` 与 `/chat/completions`，是否包含 `/v1` 由服务地址本身决定。模型列表按 OpenAI 标准读取 `data[].id`，同时容忍常见的 `models` 数组；浏览器直连受跨域限制时使用酒馆只读状态接口代发，失败时仍可手动填写模型名称。启用状态、地址、模型和密钥长期保存在当前浏览器，并可手动测试连接。总结合同固定为最近 6 条原文、每 2 个更旧楼层生成小总结、每 5 条已接受小总结生成大总结，正文上限为 600/1200 字；摘要绑定 `saveUuid + revision + 来源消息指纹` 并写入浏览器 `memory-summary-archive:v3`，迟到结果丢弃。密钥不进入 GameSnapshot、MessageArchive、提示词或日志。Tavern 记忆侧档与剧情注入仍未实现。

未来接入剧情上下文时，顺序固定为：确定性游戏状态 → 已确认角色认知 → 关系短叠层 → 长期摘要 → 最近 6 条原文 → 当前幕剧情与人物世界书。当前已接受摘要尚未进入该链；摘要只补连续性，不写回 AP、日期、事件完成或关系数值。

### 界面顺序

- 已读剧情继续走地图上的已读剧情按钮，不并入菜单“目录”。
- 本轮先把原“数据”中的只读上下文预览移到菜单“目录”，不改预览内容。
- 图三式人物档案与角色记忆成品页暂缓；现有角色档案不改。
