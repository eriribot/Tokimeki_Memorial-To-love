...
- 正式约会、放学同行、费用选择和完成提示都绝对定位在现有 `.map-section` 内；GAL 日期场景复用 `GalStoryPage` 的 30:17 舞台。约会正文、放学同行和只读历史回放只在点击 Gal 对话框的非控件区域时前进，页码、翻页按钮、链接与输入控件不会因冒泡重复推进；历史末页返回已读目录。角色页的 `expression` 由 AI 根据当页情绪从对应立绘的登记表情中自行选择；约会选项图标由独立的 `datingOptionVisuals.ts` 按稳定选项 ID 映射，AI 不生成图标或 emoji。关系结算器不读取表情或图标，因此后续扩充互动表现不需要修改数值规则。
...
- 大小总结的 TIDD-EC 提示词、自动调度、纯文本规范化、本地候选封装和人工审查已经接通：只有主存档与 MessageArchive 同次写入成功后才排队；每个当前采用的主线幕是一楼，每场已经 `completeRun` 并进入 `DatingArchive` 的完整约会也投影为一楼，进行中的约会草稿不计。约会楼层以稳定的本地 User/Assistant 消息对承载日期、对象、地点以及按正文/返程顺序保存的台词，不伪装成主线或真实宿主消息。累计 6 个未覆盖楼层形成自动小总结，手动生成可选择最早的 1–6 个未覆盖楼层。当前来源存在失败任务时后续自动批次暂停，失败任务不会自动循环，重试会重新校验已保存锚点、当前采用楼层和原文。每次调度先处理已经凑齐的 5 条大总结批次，再处理新的 6 楼层小总结批次；同一批大总结的来源指纹、楼层和消息必须互不重复。副 API 未启用时，手动小总结按钮保持 disabled/灰色；启用后只有运行中任务或与当前 save/source 仍匹配的失败任务阻断，过期失败记录不误锁新批次。副 API 只提供摘要正文；标题、来源指纹、来源 ID、状态、模型、时间戳及 JSON 存储外壳全部由本地代码生成，新候选的
  ...
  `saveUuid`；普通自动存档只有在主档与原文档成对成功后才替换记忆锚点，写入失败会继续保留上一份已配对上下文；手动默认档和读档仍使用可失效、可回滚的切换。当前读档视图额外以非持久化的 active revision、完整楼层消息对、正文与时间线指纹重验候选和任务；读回较早或较短的楼层投影时，未来总结、失败任务和大总结依赖只从当前视图与调度覆盖中退出，不删除浏览器记录，重新读回匹配的新 revision 后可以恢复。存档槽可共用 UUID，因此删除或覆盖槽位不会按 UUID 猜测清空浏览器摘要；孤立记录由当前身份和来源重验隔离。该 archive 不是 Tavern 文件侧档；接受结果尚未注入剧情生成，也不结算 AP、日期、好感、`friendship/romance/hurt`
  或约会资格。
...
| `DatingModule/`                            | 非主线邀约、日历预约、双阶段约会、长正文与三行动协议、关系账本、回忆、地图框 GAL、生成上下文投影和交互视觉映射                   | Game/Card/Player/Skill store、当前 preset、已采纳叙事、已登记选项 ID | 本地预约/结算状态、约会 prompt 与已采纳约会正文 | 让 AI 决定数值、AP、费用、日期、选项 ID 或创建宿主消息                              |
| `DatingModule/datingGenerationContext.ts`  | 把当前约会计划、关系、主段、已完成约会与最近 6 条已采纳主线/约会消息组装为可审查的确定性生成投影                                | GameSnapshot、主线 messagesave、当前 DatingRun            | 约会 `userInput`、连续性消息 ID 与只读生成参数   | 调用模型、写 store、创建宿主消息或改动任何结算                                      |
...
| `services/localContextPreview.ts`          | 本地快照、原文、当前主线/约会生成与有效玩家 Persona 投影的只读汇总                                                                | Game/Card/Player/Skill/Dating store、messagesave          | 主线/约会上下文与 Persona 预览模型              | 写回状态或触发生成                                                                  |
...
| `memory/summaryScope.ts`                   | 按当前 UUID、revision、完整楼层消息对、内容/时间线指纹和大总结依赖投影非破坏性摘要视图                                               | 当前 SaveRecord 投影、浏览器候选与任务                    | 当前可见/可操作 summaries 与 jobs              | 删除未来 revision 记录或把跨楼层记录冒充当前来源                                    |
...
| `GalMainStory/GalMainStory.tsx`            | 加载/错误/保底、历史回放、GAL 播放及派生 CG 组的本地帧游标/淡出编排；主线进度固定显示“当前幕-当前页 / 总幕数-本幕总页数”             | Store、演出 cue、场景/角色 manifest、CG 定义              | GAL 画面、CG 逐帧显隐与翻页意图                | 用全部已读页累计数替代当前幕进度、选择数据和持久化                                  |
| `GalMainStory/GalChoicePanel.tsx`          | 三项生成候选、第四项自由输入入口及独立同框输入页                                                                                   | 当前幕候选、已存选择、提交回调                            | 选择 UI 与玩家意图                             | 生成候选或推进剧情                                                                  |
| `GalMainStory/StoryHistoryArchive.tsx`     | 主线候选重生成/采用/回放/删除，以及按每场约会的 main/return 持久化回合逐层只读预览                                                  | 各幕楼层档案、DatingArchive                               | 版本管理与回放意图                             | 删除宿主聊天楼层、从约会回放改写结算或永久拒绝未采用候选之外的总结                  |
...
| `fileBridge/uploadWithCommittedReadback.ts` | 兼容宿主文件已提交但上传响应失败：仅在固定公开路径回读的完整 JSON 与预期严格一致时确认提交                                           | `/api/files/upload` 响应、固定文件回读、预期 payload      | 已验证的持久化 payload 或原上传错误             | 把未落盘、旧 revision 或部分匹配内容冒充成功                                         |
| `savesolt/index.ts`                        | Tavern 主存档文件桥；上传成功或“提交后响应失败”都必须经固定路径回读校验                                                            | `SaveRequest`、SillyTavern 文件接口                       | SaveRecord 文件                                | 对话档写入、真实宿主消息或仅凭 HTTP 状态宣告落盘                                     |
...
| `messagesolt/index.ts`                     | Tavern 对话档文件桥；上传成功或“提交后响应失败”都必须经固定路径回读校验                                                            | `MessageRequest`、SillyTavern 文件接口                    | MessageArchive 文件                            | 真实宿主消息楼层或用主档成功掩盖对话档失败                                           |
| `components/ContextPreviewModal.tsx`       | 快照/原文/主线与约会上下文/玩家 Persona 阅读、总结审查与手动按钮禁用态                                                            | 本地预览、摘要 archive/runtime、API 开关                  | 数据阅读与人工审查意图                         | 绕过 API 开关或直接调用剧情生成                                                     |
...
- 主档与对话档仍按顺序写入两个独立文件；若 SillyTavern 在文件已原子落盘后才因返回路径生成失败而回复非 2xx，两个文件桥会从固定公开路径回读，并且只有完整 JSON 与本次预期严格一致时才继续。该兼容只修复“已提交但错误响应”造成的中断，不把两个 HTTP 上传宣称为宿主事务。
